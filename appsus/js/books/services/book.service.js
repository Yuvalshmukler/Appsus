...
function query() {
  return storageService.query(BOOKS_KEY)
    // const books = storageService.query(BOOKS_KEY)
    // console.log(books)
    // return books
...
function addReview(bookId, review){
    console.log('adding review', bookId, review)
 ...
 function deleteReview(bookId,idx){
  console.log('deleting review', bookId, idx)
    bookService.query().then(books => 
        {console.log(books)
          let book = books.find(book => book.id === bookId)
          book.review.splice(idx,1)
          console.log(book)
          save(book)
        })
 ...
 function getNextBookId(bookId){
      ...
      .then(books => {
          ...
          return (idx < books.length-1)? books[idx + 1].id : cars[0].id
      })
 }
...
function _preapreData(data){
  console.log('recived data', data.items)
  var searchBooks = []
 data.items.forEach ( ({id, volumeInfo} )=> {
    console.log(id, volumeInfo)
    const {title, authors, publishedDate, description, 
      pageCount, categories, language, imageLinks } = volumeInfo
    var book = {
         "id": id,
         title,
         "subtitle": "",
